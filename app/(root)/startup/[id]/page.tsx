...
const md = markdownit();
export const experimental_ppr = true; // flag

async function page({ params }: { params: Promise<{ id: string }> }) {
  const id = (await params).id;
  const [post, { select: editorsPost }] = await Promise.all([
    client.fetch(STARTUP_BY_ID, { id }),
    client.fetch(PLAYLIST_BY_SLUG_QUERY, {
      slug: "editor-pics",
    }),
  ]); // pass two different calls
  ...
  return (
    <>
      <section className="pink_container !min-h-[230px]">
        <p>{formateDate(post?._createdAt)}</p>
        ...
        {editorsPost.length > 0 && (
          <div className="max-w-4xl mx-auto">
            ...
            <ul className="mt-7 card_grid-sm">
              {editorsPost?.map((post: StartupTypeCard, i: number) => (
